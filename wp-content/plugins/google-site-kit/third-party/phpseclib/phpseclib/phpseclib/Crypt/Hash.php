    ...
    public function hash($text)
    {
        ...
        if ($algo == 'aes_cmac') {
            ...
            if ($this->recomputeAESKey) {
                ...
                $this->c = new \Google\Site_Kit_Dependencies\phpseclib3\Crypt\AES('ecb');
                $this->c->setKey($this->key);
                $this->c->disablePadding();
                $l = $this->c->encrypt($constZero);
                $msb = ($l & "\x80") == "\x80";
                $l = new \Google\Site_Kit_Dependencies\phpseclib3\Math\BigInteger($l, 256);
                $l->setPrecision(128);
                $l = $l->bitwise_leftShift(1)->toBytes();
                // make it constant time
                $k1 = $msb ? $l ^ $constRb : $l | $constZero;
                $msb = ($k1 & "\x80") == "\x80";
                $k2 = new \Google\Site_Kit_Dependencies\phpseclib3\Math\BigInteger($k1, 256);
                $k2->setPrecision(128);
