...
class SFTP extends \Google\Site_Kit_Dependencies\phpseclib3\Net\SSH2
{
    ...
    private function get_sftp_packet($request_id = null)
    {
        ...
        $length = \unpack('Nlength', \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::shift($this->packet_buffer, 4))['length'];
        $tempLength = $length;
        ...
        if ($this->use_request_id) {
            $packet_id = \unpack('Npacket_id', \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::shift($this->packet_buffer, 4))['packet_id'];
    ...
    public function copy($oldname, $newname)
    {
        ...
        $packet = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $oldname);
        $packet .= $this->version >= 5 ? \pack('N3', 0, NET_SFTP_OPEN_OPEN_EXISTING, 0) : \pack('N2', NET_SFTP_OPEN_READ, 0);
        $this->send_sftp_packet(NET_SFTP_OPEN, $packet);
        $response = $this->get_sftp_packet();
        switch ($this->packet_type) {
        ...
        $packet = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $newname);
        $packet .= $this->version >= 5 ? \pack('N3', 0, $flags, 0) : \pack('N2', $flags, 0);
        $this->send_sftp_packet(NET_SFTP_OPEN, $packet);
        $response = $this->get_sftp_packet();
        switch ($this->packet_type) {
        ...
        $packet = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('ssQQsQ', 'copy-data', $oldhandle, 0, $size, $newhandle, 0);
        $this->send_sftp_packet(NET_SFTP_EXTENDED, $packet);
        $response = $this->get_sftp_packet();
        if ($this->packet_type != NET_SFTP_STATUS) {
            throw new \UnexpectedValueException('Expected NET_SFTP_STATUS. ' . 'Got packet type: ' . $this->packet_type);
